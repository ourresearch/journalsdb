...
class ImportISSNs:
    def __init__(self):
        self.zip_url = "https://www.issn.org/wp-content/uploads/2014/03/issnltables.zip"

    def import_issns(self):
        issn_tsv_file = self.get_zipfile()

        # ensure ISSN temp table is empty
        self.clear_issn_temp_table()

        # copy issn records into temp table
        self.copy_tsv_to_temp_table(issn_tsv_file)
        ...
        self.save_new_records(new_records)
        self.map_issns_to_issnl()
        self.clear_issn_temp_table()
    ...
    def get_zipfile(self):
        ...
        file_name = self.find_file_name(zipfile)
    ...
    def process_crossref_issns(self):
        """
        ...
        self.mark_missing_journals_as_processed(missing_journals)
        ...
        for issn in issns_to_process:
            ...
            if r:
                ...
                if not issn_exists:
                    self.save_issn_not_in_issn_org(issn)
            db.session.commit()
    ...
    def save_issn_not_in_issn_org(self, issn):
        crossref_api_issns = self.get_crossref_api_issns(issn)
